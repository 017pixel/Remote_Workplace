...
Remote Workplace verwendet jetzt die veröffentlichte React-Router-8-SPA-API
...
- Das Dashboard bindet ausschließlich an Loopback. Es gibt keinen Tailscale-Serve- oder Funnel-
  Eintrag für den Hermes-Port; die Workbench schützt `/hermes` mit Tailscale-Identität und
  Same-Origin-Prüfung.
...
- Der Hermes-Checkout bleibt außerhalb des Repositories, weil `hermes update` ihn per Git-Pull
  aktualisiert. Das Workbench-Theme liegt separat unter `HERMES_HOME/dashboard-themes`.
...
- Bei der Integration wurden die zuvor dauerhaft freigegebenen Muster für rekursives Löschen,
  ...
  `Immer erlauben`-Aktion in der Workbench-UI. Ein Installationslauf mit `--keep-allowlist` ist
