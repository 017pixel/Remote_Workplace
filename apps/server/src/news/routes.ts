...
export async function registerNewsRoutes(app:FastifyInstance,services:{news:NewsService;newsDatabase:NewsDatabase}){
  ...
  app.get("/news",async request=>newsListResponseSchema.parse(services.news.list((()=>{const q=listQuery.parse(request.query);return{limit:q.limit,...(q.q?{search:q.q}:{}),...(q.category?{category:q.category}:{}),...(q.importance?{importance:q.importance}:{}),...(q.mediaType?{mediaType:q.mediaType}:{}),...(q.saved!==undefined?{saved:q.saved}:{}),...(q.collectionId?{collectionId:q.collectionId}:{}),...(q.cursor?{cursor:q.cursor}:{})};})())));
  ...
  app.post("/news/sync",async()=>{const running=services.news.state().running;if(!running)void services.news.sync();return newsSyncResponseSchema.parse({accepted:!running,running:true});});
  app.post("/news/chat",async request=>newsChatResponseSchema.parse(await services.news.chat(...((input)=>[input.question,input.itemId] as const)(newsChatRequestSchema.parse(request.body)))));
  app.get("/news/image/:id",async(request,reply)=>{const item=services.newsDatabase.get(idParams.parse(request.params).id);if(!item.coverUrl)return imageUnavailable(reply);const url=new URL(item.coverUrl);if(!["http:","https:"].includes(url.protocol)||url.hostname==="localhost"||/^127\.|^10\.|^192\.168\.|^169\.254\.|^172\.(1[6-9]|2\d|3[01])\./.test(url.hostname))return imageUnavailable(reply);const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),8_000);try{const response=await fetch(url,{signal:controller.signal,redirect:"follow",headers:{"User-Agent":"RemoteWorkplace-TechTLDRs/0.20",Accept:"image/avif,image/webp,image/*;q=0.9"}});if(!response.ok)return imageUnavailable(reply);const data=Buffer.from(await response.arrayBuffer());if(data.byteLength===0||data.byteLength>6_000_000)return imageUnavailable(reply);const declaredType=response.headers.get("content-type")?.split(";")[0]?.trim()??"";const type=declaredType.startsWith("image/")?declaredType:detectedImageType(data);if(!type)return imageUnavailable(reply);return reply.header("Cache-Control","public, max-age=86400, stale-while-revalidate=604800").header("Content-Security-Policy","default-src 'none'").header("X-Content-Type-Options","nosniff").type(type).send(data);}catch{return imageUnavailable(reply);}finally{clearTimeout(timer);}});
