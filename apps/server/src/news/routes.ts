...
export async function registerNewsRoutes(app:FastifyInstance,services:{news:NewsService;newsDatabase:NewsDatabase}){
  ...
  app.post("/news/chat",async request=>newsChatResponseSchema.parse(await services.news.chat(...((input)=>[input.question,input.itemId,input.history] as const)(newsChatRequestSchema.parse(request.body)))));
